...
public class CBTi_Data21a {
	...
	ArrayList<CBTi_Data21c> alSleepDiary;
		
	Activity c;
	
	public CBTi_Data21a( Activity c) {
		this.c = c;  
		loadData();
	}
	...
	public void saveData() {
		try {
			...
			for( int i=0 ; i<iSize ; i++ ) {
				CBTi_Data21c cObj = alSleepDiary.get(i); 		// grab the next object and serialize it
				cObj.writeData( out );
			}
	...
	void loadData() {
		alSleepDiary= new ArrayList<CBTi_Data21c>();
		try {
			File dir = new File(c.getFilesDir(), sSubDir );		// open the subdirectory if it exists
			if(!dir.exists()) 									// create if necessary
	            dir.mkdir();	
			...
			while( iNumEntry-- > 0 ) {
				alSleepDiary.add(new CBTi_Data21c( c, in ));
			}
	...
	private void reorderSleepDiaries() {
		ArrayList<CBTi_Data21c> alSleepDiaryNew= new ArrayList<CBTi_Data21c>();
		int iIndexOfNewest;
