...
public class CBTi_Data31b {
	
	public Boolean 					bProvider = true;
	public ArrayList<CBTi_Data31c> 	alISIData;
		 
	Activity c;
	
	public CBTi_Data31b( Activity c) {
		this.c = c;  
		loadData();
	}
	...
	void loadData() {
		alISIData = new ArrayList<CBTi_Data31c>();
		try {
			File dir = new File(c.getFilesDir(), sSubDir );		// open the subdirectory if it exists
			if(!dir.exists()) 									// create if necessary
	            dir.mkdir();	
			...
			while( iNumEntry-- > 0 ) {
				CBTi_Data31c cData31c = new CBTi_Data31c(c);
	...
	public void addEntry( CBTi_Data31c cData31c ) {
